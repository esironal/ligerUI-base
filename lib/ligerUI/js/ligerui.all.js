    ...
    liger.getEditor = function (e)
    {
        ...
        var defaultOp = {
            create: function (container, editParm, controlOptions)
            {
                ...
                if (field.dictionary) //字典字段，比如:male|Female
                {
            ...
            { 
                if (text && editor.setText)
                {
                    ...
                    //如果没有把数据Save到 textField 字段，那么需要获取值字段
                else
                {
                    var field = editParm.field || editParm.column;
...
    The program will query the Dom with the liger-plugin name class name, load the corresponding parameters from dom and call the plugin.
        For example, if you encounter a .liger-grid dom, you will find the parameters required for liger.defaults.Grid loading.
        And in the config.Grid configuration of these parameter types, will dynamically load data, and columns will be set to an array
    The priority of parameter processing is:
    1,ignores ignore parameters that are not processed
    2, dom exists {property name} class name, such as <ul class="columns"></ul>, this parameter will be set to a complex property (object or array): find the corresponding defaults and config to load
         Defaults is to find $.liger.inject.defaults first, can not find and find liger.defaults
         Config is {parent configuration}.{property name}, such as config.Grid.columns
    3, directly read the data-{property name} or {property name} dom attribute
    */
    liger.inject = {
        ...
        /* 
        Naming rules: plugin name _ attribute name (including the attribute of the Nth level) (the plugin name is capitalized, the attribute name is lowercase)
        Get the rule: when you get the default, you will find it here. You can not find the liger.defaults, such as liger.defaults.Grid_columns.
        Note: Only the list of parameters is defined here.
        */
        defaults: {
            Grid_detail: {
                height: null,
        ...
        /*
        The config is configured with a plugin parameter or a type of complex attribute parameter (dynamic loading, array, default parameter)
        */
        config: {
            Grid: {
                //动态
...
(function ($)
{
    ...
    $.ligerDefaults.ComboBox = {
        ...
        condition: null,       //List条件search for 参数同 ligerForm
        grid: null,              //form 参数同 ligerGrid
        onStartResize: null,
        onEndResize: null,
        ...
        url: null,              //数据源URL(需返回JSON)
        urlParms: null,                     //url带参数
        selectBoxRender: null,       //自定义selectbox的content
        selectBoxRenderUpdate: null,  //自定义selectbox(发生值改变)
        detailEnabled : true,       //detailUrl是否有效
        detailUrl: null,            //determine选项的时候，使用这个detailUrl加载到详细的数据
        ...
        onError: null,
        onBeforeOpen: null,      //打开下拉框前事件，可以通过return false来阻止继续操作，利用这个参数可以用来调用other函数，比如打开一个新窗口来选择值
        onButtonClick: null,      //右侧图标按钮事件，可以通过return false来阻止继续操作，利用这个参数可以用来调用other函数，比如打开一个新窗口来选择值
        onTextBoxKeyDown: null,
        onTextBoxKeyEnter : null,
        render: null,            //文本框显示html函数
        absolute: true,         //选择框是否在附加到body,并绝对定位
        cancelable: true,      //可cancel选择
        css: null,            //附加css
        parms: null,         //ajax提交表单 
        renderItem: null,   //选项自定义函数
        autocomplete: false,  //自动完成 
        autocompleteAllowEmpty: true, //是否允许空值search for
        ...
        conditionSearchClick: null,      //下拉框formsearch for按钮自定义函数
        onChangeValue: null,
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            //update by superzoc increase初始值
                ...
                success: function (result)
{
        ...
        updateSelectBoxPosition: function ()
        {
            ...
            if (p && p.absolute)
            {
                ...
                {
                    //若下拉框大小超过当前document下边框,且当前document上留白大于下拉content高度,下拉content向上展现
...
(function ($)
{
    ...
    $.ligerDefaults.Dialog = {
        ...
        urlParms: null,     //传参
        load: false,     //是否以load()的方式加载目标页的content 
        ...
        allowClose: true, //允许shut down
        opener: null,
        timeParmName: null,  //是否给URL后面加上值为new Date().getTime()的参数，如果需要指定一个参数名即可
        closeWhenEnter: null, //回车时是否shut downdialog
        isHidden: true,        //shut down对话框时是否只是隐藏，还是销毁对话框
    ...
    $.ligerDefaults.DialogString = {
        ...
        no: '否',
        cancel: 'cancel',
        waittingMessage: '正在等待中,Please wait...'
    };
            ...
            else if (p.url)
            {
                ...
                else
                {
                    ...
                    setTimeout(function ()
                    {
                        ...
                        g.jiframe[0].dialog = g;//increase窗口对dialog对象的引用
                        /*
                        可以在子窗口这样使用：
                        var dialog = frameElement.dialog;
                        var dialogData = dialog.get('data');//获取data参数
                        dialog.set('title','新标题'); //设置标题
                        dialog.close();//shut downdialog 
                        */
...
(function ($)
{
    ...
    $.ligerDefaults.Form_fields = {
        ...
        afterContent: null,  //后置content
        beforeContent : null, //前置content
        hideSpace: false,
        hideLabel: false,
        rightToken: null,        
        attrRender: null,
    ...
    $.ligerui.controls.Form.ligerExtend($.ligerui.core.UIComponent, {
            ...
            if (fields && fields.length)
            {
                ...
                $(groups).each(function (groupIndex, group)
                {
                    ...
                    { 
                        ...
                        out.push('><ul>');
                        //append field 编辑后面自定义content
                        if (field.beforeContent) //前置content
                        {
        ...
        getChanges: function ()
        {
            ...
            //注意!! getData会导致数据被Refresh. 必须严格控制getData的调用. 
            //调用本函数不会导致Refresh数据. 
            var g = this, p = this.options;
...
(function ($)
{
    ...
    $.ligerDefaults.Grid = {
        ...
        data: null,                            //初始化数据
        usePager: true,                         //是否分页
        hideLoadButton : false,                 //是否隐藏Refresh按钮
        ...
        groupColumnDisplay: '分组',             //分组 - 列显示first name
        ...
        toolbar: null,                           //Toolbar,参数同 ligerToolbar的,额外参数有title、icon
        toolbarShowInLeft: true,               //Toolbar显示在左边
        headerImg: null,                        //form头部图标  
        ...
        onUnSelectRow: null,                   //cancel选择行事件
        ...
        onError: null,                         //错误事件
        onSubmit: null,                         //提交前事件
        onReload: null,                    //Refresh事件，可以通过return false来阻止操作
        ...
        onAfterAddRow: null,                     //increase行后事件
        ...
        onBeforeCancelEdit: null,                 //cancel编辑前事件
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _setHeight: function (h)
        {
            ...
            if (h > 0)
            {
                ...
                if (p.frozen)
                { 
                    //解决冻结列和活动列由上至下滚动Misplaced question
        ...
        loadData: function (loadDataParm,sourceType)
        {
            ...
            if (p.dataType == "local")
            {
                ...
                //该语句modify了p.data, 导致过滤数据后, 丢失了初始数据.
        ...
        */
        ...
        getChanges: function ()
        {
            //getChanges函数必须保留__status属性,否则根本不知道哪些是新增的,哪些是被delete的.
            //则本函数返回的结果毫无意义.
            var g = this, p = this.options;
            var data = [];
            if (this.deletedRows)
        ...
        changePage: function (ctype)
        {
            ...
            {
                g.currentData = g._getCurrentPageData(g.filteredData);
                //increase以下一句调用: 在显示数据之前, 应该先调用转换tree的函数. 
                //否则会导致树的数据重复显示
                if (p.tree)
                {
        ...
        _initColumns: function ()
        {
            ...
            {
                var col = p.columns[i];
                //increase以下一句. 因为在低版本IE中, 可能因为modify了prototype, 
        ...
        _getSrcElementByEvent: function (e)
        {
            ...
            var r = {
                ...
                gridheader: fn("l-grid-header"), //form头 
        ...
        _onClick: function (e)
        {
            ...
            else if (src.groupbtn)
            {
                ...
                while (true)
                {
                    ...
                    {
                        currentRow.show();
                        //如果是明细展开的行，并且之前的状态已经是shut down的，隐藏之
        ...
        showFilter: function ()
        {
            ...
            return g.winfilter = $.ligerDialog.open({
                ...
                buttons: [
					{ text: 'determine', onclick: function (item, dialog) { loadData(); dialog.hide(); } },
...
(function ($)
{
    ...
    $.ligerui.controls.Layout.ligerExtend($.ligerui.core.UIComponent, {
        ...
        //加了centerBottom以后，需要对centerBottom进行Refresh处理一下
...
(function ($)
{
    ...
    $.ligerMessageBox.alert = function (title, content, type, onBtnClick)
    {
        ...
        p = {
            title: title,
            content: content,
            buttons: [{ text: 'determine', onclick: onclick}]
        };
        if (type) p.type = type;
        return $.ligerMessageBox(p);
    };
...
(function ($)
{
    ...
    $.ligerDefaults.Panel = {
        ...
        url: null,          //远程contentUrl 
        urlParms: null,     //传参
        frameName: null,     //创建iframe时 作为iframe的name和id 
        data: null,          //可用于传递到iframe的数据 
        showClose: false,    //是否显示shut down按钮
        showToggle: true,    //是否显示收缩按钮 
        showRefresh: false,    //是否显示Refresh按钮
        icon: null,          //左侧按钮
        onClose:null,       //shut down前事件
        onClosed:null,      //shut down事件
        ...
        _setUrl: function (url)
        {
            ...
            if (url)
            {
                ...
                if(!g.jiframe)
                {
                    ...
                    g.jiframe[0].panel = g;//increase窗口对panel对象的引用
...
(function ($)
{
    ...
    $.ligerDefaults.PopupEdit = {
        ...
        css: null,             //附加css
        onButtonClick: null,    //利用这个参数来调用other函数，比如打开一个新窗口来选择值 
        ...
        split: ';',
        data: [],
        grid: null,       //在可查询、可分页List的弹出框 中选择值 
        ...
        valueFieldCssClass: null,
        searchClick: null      //弹窗查询search for按钮自定义函数
    };
    ...
    //创建一个可查询、可分页List的选取弹出框 需要dialog,grid,form等插件的支持
    $.ligerui.getPopupFn = function (p,master)
    {
        p = $.extend({
            ...
            left: null,
            split: ';',
            valueField: null,    //接收form的value字段名
            textField: null,     //接收form的text字段名
            grid: null,          //form的参数 同ligerGrid
            condition: null,     //search for表单的参数 同ligerForm
...
(function ($)
{
    ...
    $.ligerDefaults.Portal = {
        ...
        rows:[
            {columns:[ 
                {
                   width : '50%',
                   panels : [{width:'100%',content:'content'},{width:'100%',url:@url1}]
                },{
                   width : '50%',
    ...
    $.ligerui.controls.Portal.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _init: function ()
        {
            ...
            if ($(">div", g.element).length) //如果已经定义了DIV子元素,那么这些元素将会转换为columns,这里暂时Save到tempInitPanels
            { 
                p.columns = [];
...
(function ($)
{
    ...
    $.ligerDefaults.Tab = {
        ...
        changeHeightOnResize: false,
        contextmenu: true,
        dblClickToClose: false, //是否双击时shut down
    ...
    $.ligerDefaults.TabString = {
        closeMessage: "shut down当前页",
        closeOtherMessage: "shut downother",
        closeAllMessage: "shut down所有",
    ...
    $.ligerui.controls.Tab.ligerExtend($.ligerui.core.UIComponent, {
        ...
        addTabItem: function (options)
        {
            ...
            if (url)
            {
                iframe[0].tab = g;//increaseiframe对tab对象的引用  
                if (options.data)
                {
...
(function ($)
{
    ...
    //shut down指定在Dom Element(附加了ligerui对象ID,属性名"ligeruitipid")显示的bubble
    ...
    $.ligerDefaults.Tip = {
        ...
        x: 0,
        y: 0,
        appendIdTo: null,       //SaveID到那一个对象(jQuery)(待移除)
        target: null,
        auto: null,             //是否自动模式，如果是，那么：鼠标经过时显示，移开时shut down,并且当content为空时自动读取attr[title]
        removeTitle: true        //自动模式时，默认是否移除掉title
    };

    //在指定Dom Element右侧显示bubble,通过$.fn.ligerTip调用
...
(function ($)
{
    ...
    $.ligerDefaults.Tree = {
        ...
        single: false,               //是否单选
        needCancel: true,			//已选的是否需要cancel操作
        ...
        topParentIDValue: 0,
        onBeforeAppend: function () { },        //加载数据前事件，可以通过return falsecancel操作
        ...
        是否展开 
            1,可以是true/false 
            2,也可以是数字(层次)N 代表第1层到第N层都是展开的，other收缩
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        append: function (parentNode, newdata, nearNode, isAfter)
        {
            ...
            if (parentNode == null)//increase到根节点
            {
        ...
        selectNode: function (selectNodeParm,isTriggerEvent)
        {
            ...
            $("li", g.tree).each(function ()
            {
                ...
                {
                    //修复多选checkbox为true时调用该方法会cancel已经选中节点的问题
